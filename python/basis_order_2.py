...
def N_order2( knots : [float], k : int, u : float ) -> float:

    '''Basis function of order 2 (quadratic).
       :param knots : knots array
       :param k: knot index.
       :param u: input
       :returns: output       
    ...
    if (knots[k+2] - knots[k]) == 0.0:
        sum1 = 0.0
    else:
        sum1 =  (u - knots[k]) / (knots[k+2] - knots[k]) * N_order1(knots, k, u)

    if (knots[k+3] - knots[k+1]) == 0.0:
        sum2 = 0.0
    else:
        sum2 = (knots[k+3] - u) / (knots[k+3] - knots[k+1]) * N_order1(knots, k+1, u)
...
def plot_one_func( ax, knot_index ):
...
for k in range( 0, len(knots) - 3 ):
    plot_one_func( ax, k )

ax.plot( knots, [0] * 14, 'o' )

#ax.set_title( 'Basis Function Order 2' )
...
plt.savefig('basis_order_2.png', dpi=300)
